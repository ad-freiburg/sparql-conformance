...
def generate_highlighted_string_sv(array: list, remaining: list, mark_red: list, result_type: str) -> str:
    """
...
def compare_values(value1: str, value2: str, use_config: bool, alias: dict, map_bnodes: dict) -> bool:
    """
    ...
    if len(value1) > 1 and len(value2) > 1 and value1[0] == "_" and value2[0] == "_":
        if value1 not in map_bnodes and value2 not in map_bnodes:
            ...
            return True
        if map_bnodes.get(value1) == value2 and map_bnodes.get(value2) == value1:
            return True
        return False
...
def compare_rows(row1: list, row2: list, use_config:bool, alias: dict, map_bnodes: dict) -> bool:
    """
    ...
    for element1, element2 in zip(row1, row2):
        if not compare_values(element1.split("^")[0], element2.split("^")[0], use_config, alias, map_bnodes):
            return False
    return True

def compare_array(expected_result: list, result: list, result_copy: list, expected_result_copy: list, use_config: bool, alias: dict, map_bnodes: dict):
    """
...
def compare_sv(expected_string: str, query_result: str, result_format: str, alias: dict):
    """
    ...
    compare_array(expected_array, actual_array, actual_array_copy, expected_array_copy, False, alias, map_bnodes)
    ...
    if len(actual_array_copy) == 0 and len(expected_array_copy) == 0:
        ...
        compare_array(expected_array_copy, actual_array_copy, actual_array_mark_red, expected_array_mark_red, True, alias, map_bnodes)
        if len(actual_array_mark_red) == 0 and len(expected_array_mark_red) == 0:
            status = INTENDED
            error_type = INTENDED_MSG
    
    expected_html = generate_highlighted_string_sv(expected_array, expected_array_copy, expected_array_mark_red, result_format)
    actual_html = generate_highlighted_string_sv(actual_array, actual_array_copy, actual_array_mark_red, result_format)
    expected_html_red = generate_highlighted_string_sv(expected_array_copy, expected_array_copy, expected_array_mark_red, result_format)
    actual_html_red = generate_highlighted_string_sv(actual_array_copy, actual_array_copy, actual_array_mark_red, result_format)
