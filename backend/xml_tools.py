...
def element_to_string(element: ET.Element, escaped_xml: str, label: str):
    ...
    if escaped_element_str in escaped_xml:
        ...
        return highlight_first_occurrence(escaped_xml, escaped_element_str.replace('&quot;', "&apos;"), label)
    else:
...
def xml_elements_equal(element1: ET.Element, element2: ET.Element, compare_with_intended_behaviour: bool, alias: dict, number_types: list, map_bnodes: dict) -> bool:
    """
    ...
    if element1.tag != element2.tag:
        if (alias.get(element1.tag) != element2.tag and alias.get(element2.tag) != element1.tag) or not compare_with_intended_behaviour: 
            return False

    if element1.attrib != element2.attrib: 
        if isinstance(element1.attrib, dict) != isinstance(element2.attrib, dict): return False
        if not isinstance(element1.attrib, dict) and (alias.get(element1.attrib) != element2.attrib and alias.get(element2.attrib) != element1.attrib) or not compare_with_intended_behaviour: return False
        if isinstance(element1.attrib, dict):
            if element1.attrib.get("datatype") is None and element2.attrib.get("datatype") is None:
                return False
            else:
                if (alias.get(element1.attrib.get("datatype")) != element2.attrib.get("datatype") and alias.get(element2.attrib.get("datatype")) != element1.attrib.get("datatype")) or not compare_with_intended_behaviour: return False
    ...
    if element1.text != element2.text:
        if element1.tag == "{http://www.w3.org/2005/sparql-results#}bnode":
            if element1.text not in map_bnodes and element2.text not in map_bnodes:
                ...
                return all(xml_elements_equal(c1, c2, compare_with_intended_behaviour, alias, number_types, map_bnodes) for c1, c2 in zip(element1, element2))
        ...
        if element1.text.strip() == element2.text.strip(): return all(xml_elements_equal(c1, c2, compare_with_intended_behaviour, alias, number_types, map_bnodes) for c1, c2 in zip(element1, element2))
        if is_number:
            if float(element1.text) == float(element2.text): return all(xml_elements_equal(c1, c2, compare_with_intended_behaviour, alias, number_types, map_bnodes) for c1, c2 in zip(element1, element2))
        if (alias.get(element1.text) != element2.text and alias.get(element2.text) != element1.text) or not compare_with_intended_behaviour: return False
    
    return all(xml_elements_equal(c1, c2, compare_with_intended_behaviour, alias, number_types, map_bnodes) for c1, c2 in zip(element1, element2))

def xml_remove_equal_elements(parent1: ET.Element, parent2: ET.Element, use_config: bool, alias: dict, number_types: list, map_bnodes: dict):
    """
    ...
    for child1 in list(parent1):
        for child2 in list(parent2):
            if xml_elements_equal(child1, child2, use_config, alias, number_types, map_bnodes):
                parent1.remove(child1)
                parent2.remove(child2)
                break

def compare_xml(expected_xml: str, query_xml: str, alias: dict, number_types: list) -> tuple:
    """
    ...
    if head1 is not None and head2 is not None:
        xml_remove_equal_elements(head1, head2, False, alias, number_types, map_bnodes)
    ...
    if results1 is not None and results2 is not None:
        xml_remove_equal_elements(results1, results2, False, alias, number_types, map_bnodes)
    # Copy expected_tree
    ...
    if (results1 is not None and results2 is not None and len(list(results1)) == 0 and len(list(results2)) == 0 and len(list(head1)) == 0 and len(list(head2)) == 0) or (results1 is None and results2 is None and head1 is None and head2 is None and bool1 is None and bool2 is None):
        ...
        if results1 is not None and results2 is not None:
            xml_remove_equal_elements(results1, results2, True, alias, number_types, map_bnodes)
